...
public interface ControlInterface extends View.OnLongClickListener, GrabListener {
    ...
    default void setBackground() {
        if (getProperties().buttonImagePath != null && new java.io.File(getProperties().buttonImagePath).exists()) {
            if (getProperties().buttonImageDisplayMode == 1) {
                cacheSkinDrawable(loadSkinDrawable(getProperties().buttonImagePath));
            } else {
                Drawable customD = loadSkinDrawable(getProperties().buttonImagePath);
                if (customD != null) {
                    getControlView().setBackground(customD);
                    return;
                }
            }
        }
    ...
    default Drawable loadSkinDrawable(String path) {
        if (path.toLowerCase(Locale.ROOT).endsWith(".gif")) {
            try {
                return new GifDrawable(path);
            } catch (IOException e) {
                Log.w("ControlInterface", "Failed to load GIF skin", e);
            }
        }
        return Drawable.createFromPath(path);
    }

    default void cacheSkinDrawable(Drawable d) {
        getControlView().setTag(d);
    }

    default Drawable getCachedSkinDrawable() {
        Object tag = getControlView().getTag();
        if (tag instanceof Drawable) return (Drawable) tag;
        return null;
    }

    default void updateSkinForDisplayMode(boolean pressed) {
        if (getProperties().buttonImageDisplayMode == 1 && getProperties().buttonImagePath != null) {
            if (pressed) {
                Drawable skin = getCachedSkinDrawable();
                if (skin == null) {
                    skin = loadSkinDrawable(getProperties().buttonImagePath);
                    cacheSkinDrawable(skin);
                }
                if (skin != null) {
                    getControlView().setBackground(skin);
                    return;
                }
            }
            setBackground();
