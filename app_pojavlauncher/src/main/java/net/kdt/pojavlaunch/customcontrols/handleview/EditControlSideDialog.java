...
public class EditControlSideDialog extends SideDialogView {
    ...
    private Spinner mSkinDisplayModeSpinner;
    private final TextView[] mKeycodeTextviews = new TextView[4];
    private SeekBar mStrokeWidthSeekbar, mCornerRadiusSeekbar, mAlphaSeekbar, mSizeSeekbar;
    ...
    public static SkinPickListener sSkinPickListener;
    ...
    public void loadValues(ControlData data) {
        ...
        mAspectR = data.getHeight() / Math.max(1f, data.getWidth());
        mSizeSeekbar.setProgress((int) Tools.pxToDp(data.getWidth()));
        ...
        mSkinDisplayModeSpinner.setSelection(data.buttonImageDisplayMode);
    ...
    private void loadAdapter() {
        ...
        // Skin display mode spinner
        ArrayAdapter<CharSequence> skinDisplayAdapter = ArrayAdapter.createFromResource(mDialogContent.getContext(),
                R.array.skin_display_modes, android.R.layout.simple_spinner_item);
        skinDisplayAdapter.setDropDownViewResource(android.R.layout.simple_list_item_single_choice);
        mSkinDisplayModeSpinner.setAdapter(skinDisplayAdapter);
    }
    ...
    private void bindLayout() {
        ...
        mSkinDisplayModeSpinner = mDialogContent.findViewById(R.id.editSkinDisplayMode_spinner);
        ...
        mSelectButtonSkin = mDialogContent.findViewById(R.id.editButtonSkin_textView);
        mSizeSeekbar = mDialogContent.findViewById(R.id.editSize_seekbar);
    ...
    private void setupRealTimeListeners() {
        ...
        mSizeSeekbar.setOnSeekBarChangeListener((SimpleSeekBarListener) (seekBar, progress, fromUser) -> {
            if (internalChanges) return;
            if (progress < 10) progress = 10;
            float newWidth = Tools.dpToPx(progress);
            float newHeight = Tools.dpToPx(progress * mAspectR);
            mCurrentlyEditedButton.getProperties().setWidth(newWidth);
            mCurrentlyEditedButton.getProperties().setHeight(mCurrentlyEditedButton.getProperties() instanceof ControlJoystickData ? newWidth : newHeight);

            internalChanges = true;
            mWidthEditText.setText(String.valueOf(mCurrentlyEditedButton.getProperties().getWidth()));
            mHeightEditText.setText(String.valueOf(mCurrentlyEditedButton.getProperties().getHeight()));
            internalChanges = false;

            mCurrentlyEditedButton.updateProperties();
        });

        mSelectButtonSkin.setOnClickListener(v -> {
            if (sSkinPickListener != null && mCurrentlyEditedButton != null) {
                sSkinPickListener.onSkinPickRequested(mCurrentlyEditedButton);
            }
        });

        mSkinDisplayModeSpinner.setOnItemSelectedListener((SimpleItemSelectedListener) (parent, view, position, id) -> {
            if (internalChanges) return;
            mCurrentlyEditedButton.getProperties().buttonImageDisplayMode = position;
