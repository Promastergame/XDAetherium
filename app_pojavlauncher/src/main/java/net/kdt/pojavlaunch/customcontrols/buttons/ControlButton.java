...
public class ControlButton extends TextView implements ControlInterface {
    ...
    private final Paint mPressOverlayPaint = new Paint();
    ...
    public ControlButton(ControlLayout layout, ControlData properties) {
        ...
        mPressOverlayPaint.setColor(Color.BLACK);
        mPressOverlayPaint.setAlpha(60);
    ...
    public void setActivated(boolean activated) {
        ...
        float targetScale = activated ? 0.94f : 1.0f;
    ...
    public void sendKeyPresses(boolean isDown) {
        ...
        post(() -> updateSkinForDisplayMode(isDown));
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isActivated())
            canvas.drawRoundRect(0, 0, getWidth(), getHeight(), mComputedRadius, mComputedRadius, mPressOverlayPaint);

        // Shift text down by 1.5dp on press/activation to feel exactly like Minecraft physical button clicks
        boolean pressed = mIsToggled || (!mProperties.isToggle && isActivated());
        if (pressed) {
            canvas.save();
            canvas.translate(0, Tools.dpToPx(1.5f));
            super.onDraw(canvas);
            canvas.restore();
        } else {
            super.onDraw(canvas);
