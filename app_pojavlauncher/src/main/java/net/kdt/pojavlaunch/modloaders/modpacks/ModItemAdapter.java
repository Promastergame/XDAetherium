...
public class ModItemAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements TaskCountListener {
    ...
    private boolean mIsCustomModpack;
    private String mProfileId;

    public void setCustomModpackConfig(boolean isCustomModpack, String profileId) {
        this.mIsCustomModpack = isCustomModpack;
            ...
            view.setOnClickListener(v -> {
                if(!hasExtended()){
                    ...
                    mExtendedButton.setOnClickListener(v1 -> {
                        if (mIsCustomModpack) {
                            android.widget.Toast.makeText(mExtendedButton.getContext(), "Поиск зависимостей...", android.widget.Toast.LENGTH_SHORT).show();
                            int selectedPos = mExtendedSpinner.getSelectedItemPosition();
                            String selectedVersionId = null;
                            if (mModDetail != null && mModDetail.versionIds != null && selectedPos >= 0 && selectedPos < mModDetail.versionIds.length) {
                                selectedVersionId = mModDetail.versionIds[selectedPos];
                            }
                            final String finalVersionId = selectedVersionId;
                            
                            new Thread(() -> {
                                net.kdt.pojavlaunch.modloaders.modpacks.api.ModDependencyResolver resolver = new net.kdt.pojavlaunch.modloaders.modpacks.api.ModDependencyResolver();
                                net.kdt.pojavlaunch.value.launcherprofiles.MinecraftProfile profile = net.kdt.pojavlaunch.value.launcherprofiles.LauncherProfiles.mainProfileJson.profiles.get(mProfileId);
                                net.kdt.pojavlaunch.modloaders.modpacks.api.ResolveResult res = resolver.resolve(
                                    mModItem.id, 
                                    profile, 
                                    mSearchFilters != null ? mSearchFilters.mcVersion : null, 
                                    mSearchFilters != null ? mSearchFilters.modLoader : null,
                                    finalVersionId
                                );
                                
                                Tools.runOnUiThread(() -> {
                                    if (res.success) {
                                        android.widget.Toast.makeText(mExtendedButton.getContext(), "Скачиваем мод и зависимости (" + res.dependenciesToInstall.size() + " шт.)", android.widget.Toast.LENGTH_LONG).show();
                                        net.kdt.pojavlaunch.modloaders.modpacks.api.SingleModInstaller.installModWithDependencies(res, mProfileId, () -> {
                                            Tools.runOnUiThread(() -> android.widget.Toast.makeText(mExtendedButton.getContext(), "Установлено!", android.widget.Toast.LENGTH_SHORT).show());
                                        }, () -> {
                                            Tools.runOnUiThread(() -> android.widget.Toast.makeText(mExtendedButton.getContext(), "Ошибка загрузки", android.widget.Toast.LENGTH_SHORT).show());
                                        });
                                    } else {
                                        android.widget.Toast.makeText(mExtendedButton.getContext(), "Ошибка: " + res.errorMessage, android.widget.Toast.LENGTH_LONG).show();
                                    }
                                });
                            }).start();
                        } else {
