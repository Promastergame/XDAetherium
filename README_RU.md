...
- **Премиум-установщик загрузчиков модов**:
  - Полностью переработан с нуля: базовые чекбоксы заменены на великолепный плиточный Minecraft-интерфейс с атмосферным фоном и аккуратной типографикой.
  - Умная фильтрация версий под каждую выбранную платформу загрузчиков.
  - Полноценная поддержка загрузки Fabric, Quilt, NeoForge, Forge и OptiFine.
  - **Полностью удален устаревший загрузчик BTA (Better Than Adventure)**, так как в него никто уже не играет и никто про него не знает.
  - **Добавлена база для загрузчика BTW (Better Than Wolves)** (в процессе разработки — каркас интерфейса полностью готов, логика скачивания и совместимости появится уже завтра-послезавтра!).
- **Магазин кастомных курсоров и пиксельный редактор**:
  ...
  - **Встроенный профессиональный пиксельный редактор**:
...
- **Продвинутое кастомное управление и кастомизация фона**:
  - **Персонализированный фон раскладки (Бета)**: возможность устанавливать собственные фоновые изображения или фотографии прямо под оверлей кастомного управления. Добавлена удобная палитра выбора цветов (в следующих обновлениях планируются еще более крутые улучшения!).
